    ...
    if sampler == 'softmax':
        ...
        def loss_func(score, feat, target, target_cam, model=None):
            if cfg.MODEL.METRIC_LOSS_TYPE == 'triplet':
                ...
                # ---------------- Proxy-Anchor (optional) ----------------
                PA_LOSS = None
                pa_on = (text_cfg is not None) and bool(getattr(text_cfg, "PROXY_ANCHOR_ON", False))
                pa_w = float(getattr(text_cfg, "PROXY_ANCHOR_W", 0.0)) if pa_on else 0.0
                if pa_on and pa_w > 0.0 and (model is not None):
                ...
                loss_func._last = {
                    ...
                    "pa_w": float(pa_w) if pa_on else 0.0,
